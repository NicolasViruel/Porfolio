export const SKILLS = [
    {
        title: "Frontend",
        icon: "../src/assets/icon/front.svg",
        skills: [
            { skill: "HTML5", percentage: "80%" },
            { skill: "CSS3", percentage: "70%" },
            { skill: "JavaScript", percentage: "70%" },
            { skill: "React", percentage: "50%" },
            { skill: "Angular", percentage: "40%" },
        ],
    },
    {
        title: "Backend",
        icon: "../src/assets/icon/backend.svg",
        skills: [
            { skill: "Nest JS", percentage: "40%" },
            { skill: "Node JS", percentage: "60%" },
            { skill: "Express JS", percentage: "50%" },
        ],
    },
    {
        title: "Database",
        icon: "../src/assets/icon/database.svg",
        skills: [
            { skill: "MySQL", percentage: "40%" },
            { skill: "MongoDB", percentage: "50%" },
            { skill: "Firebase", percentage: "40%" },
        ],
    },
    {
        title: "SoftSkills",
        icon: "../src/assets/amigos.png",
        skills: [
            { skill: "Trabajo en equipo", percentage: "80%" },
            { skill: "Comunicación", percentage: "90%" },
            { skill: "Resolución de problemas", percentage: "60%" },
            { skill: "Creatividad", percentage: "80%" },
        ],
    }

]

export const WORK_EXPERIENCE = [
    {
        title: "Junior FullStak en SILICE.SI",
        date: "Noviembre 2023 - Presente",
        responsabilities: [
            "Desarrollo Fullstack trabajando con JavaScript, React, Angular, Nest.js, Node.js, Docker, Microservicios",
            "Diseño y creacion de aplicaciones Web escalables y de alto rendimiento.",
            "Participo activamente en la fase de planificación y diseño de proyectos, garantizando la alineación con los objetivos del negocio y las mejores prácticas de desarrollo.",
            "Trabajo en estrecha colaboración con equipos multidisciplinarios para asegurar la entrega oportuna y exitosa de proyectos."
        ]
    },
    {
        title: "Tutor Fullstack en Rolling Code",
        date: "Febrero 2023 - Presente",
        responsabilities: [
            "He tenido el privilegio de trabajar en varios roles que han enriquecido mi conjunto de habilidades. Dictando clases como tutor en algunas Ocaciones",
            "Esta experiencia me ha permitido cultivar tanto mis habilidades técnicas como mis capacidades de comunicación, ya que interactúo diariamente con estudiantes con diversos niveles de experiencia.",
            "Con más de 1 añó de experiencia en la industria y un enfoque comprometido hacia el aprendizaje continuo,",
            "Especialidad de enseñanza MERN."
        ]
    },
    {
        title: "Freelance",
        date: "Abril 2022 - Presente",
        responsabilities: [
            "AMD Models - Agencia de Modelaje: Tecnologías utilizadas: React en el frontend, Node.js en el backend.",
            // "Desarrollo de un sitio web para AMD Models, una agencia de modelaje.",
            // "Base de datos: Firebase.",
            "BEER - Cervezeria: Desarrollo de una plataforma para un bar que incluye funcionalidad de carrito de compras.",
            "Red de Administrador y Login: Creación de una red de administrador para un bar u otro tipo de negocio. CRUD",,
            // "Detalles específicos del proyecto no proporcionados.",
            "Route 66 - Agencia de Motos: Desarrollo de un sitio web para una agencia de motos.",
            "KangreBurgers Hamburguesas: Creación de una página web para un bar que se especializa en hamburguesas.",
            "Detalles específicos del proyecto no proporcionados.",
        ]
    }
]

          
            